...
getnewSpeedatHill f angle m g fs step v0 cw a p bouncefac up =
    let parts = getParts (fst v0) (snd v0) angle (angle+270)
        vt  |getyPart v0 < 0 && up || getyPart v0 > 0 && (not up) = getresVector [fst parts, ((fst (snd parts) * bouncefac), (snd (snd parts)))]
...
simulateAndWriteFromFile::String->String -> Bool->IO ()
simulateAndWriteFromFile datafile resultfile vector = do
    x <- simulatefromFiles datafile vector
...
betterAngle::Double->Double
betterAngle a   | a > 360 = betterAngle (a-360)
